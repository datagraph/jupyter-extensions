...
export interface SparqlGenerator {
    ...
    filter(form:FilterPattern) : string;
    graph(form:GraphPattern) : string;
    group(form:GroupPattern, option:boolean) : string;
    optional(form:BlockPattern) : string;
    service(form:ServicePattern) : string;
    union(form:BlockPattern) : string;
    values(form:ValuesPattern) : string;
}
